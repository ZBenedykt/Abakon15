...
namespace Abakon15.ViewModels
{
    public class DocumentVM : ViewModelBase
    {
                ...
                if (m_clearDocumentsCommand == null)
                {
                    m_clearDocumentsCommand = new RelayCommand(
                        param =>
                        {
                            ...
                            List<Kalibracja> kalibracje = Kalibracja.LoadAll();
                            foreach (var kal in kalibracje)
                            {
                                if (!FilesUtility.ifFileExist(kal.ExcelFile.filePath.Path, kal.ExcelFile.FileName))
                                {
                                    deletedFiles.Add(kal.ExcelFile);
                                }
                            }
                            if (gr != null)
                            {
        ...
        EquipmentDocument curentEQDocument;

        public EquipmentDocument CurentEQDocument
        {
            get { return curentEQDocument; }
            set { SetField(ref curentEQDocument, value, () => CurentEQDocument); }
        }
            ...
            get
            {
                if (m_PrzyrzadPomiarowySingleWindowOpenCommand == null)
                {
                    m_PrzyrzadPomiarowySingleWindowOpenCommand = new RelayCommand(
                                                    param =>
                                                    {
                                                        PrzyrzadPomiarowyVM dataContext = new PrzyrzadPomiarowyVM(CurentEQDocument.przyrzadPomiarowy);
                                                        WindowManagerClass.WindowOpener<PrzyrzadPomiarowySingleWindow>(WindowContextEnum.documents, false, false, dataContext, winState: WindowState.Normal);
                                                    },
                                                    param => CurentEQDocument != null
                                                    );
                }
                ...
                if (m_ConnectPrzyrzadToDocumentCommand == null)
                {
                    m_ConnectPrzyrzadToDocumentCommand = new RelayCommand(
                                                    param =>
                                                    {
                                                        ...
                                                        dataContext.CurrentDocument = CurrentDocument;
                                                        var win = WindowManagerClass.WindowOpener<PrzyrzadPomiarowySelectFOrDocumentWindow>(WindowContextEnum.measuringDevices, true, true, dataContext);
                                                        DataGrid gr = param as DataGrid;
                                                        if (gr != null) gr.Items.Refresh();
                                                    },
                                                    param => CurrentDocument != null
                                                    );
                }
